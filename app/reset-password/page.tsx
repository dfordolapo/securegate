...
function ResetPasswordForm() {
    ...
    if (!token) {
        return (
            <div className="flex w-[350px] flex-col gap-4 rounded-xl bg-zinc-900 p-6">
                <h1 className="text-2xl font-bold text-red-500">Invalid Link</h1>
                <p className="text-zinc-300">It looks like the password reset link is missing a token. Please request a new one.</p>
                <Link href="/forgot-password" className="rounded-md bg-white p-3 font-semibold text-black text-center mt-4">
                    Request New Link
                </Link>
            </div>
        );
    ...
    return (
        ...
        >
            ...
            {isSuccess && (
                <Link href="/login" className="rounded-md bg-white p-3 font-semibold text-black text-center mt-2">
                    Go to Login
                </Link>
            )}
        </form>
...
export default function ResetPasswordPage() {
    return (
        <div className="flex min-h-screen items-center justify-center bg-black text-white">
            <Suspense fallback={
                <div className="flex w-[350px] flex-col gap-4 rounded-xl bg-zinc-900 p-6 items-center justify-center">
